...
class StochasticLearningPing extends Thread implements PacketReceiveListener,
	...
	private long receivedMessageHandlingTime;
	...
	private static class PingAdministration {
		...
		public void registerPeerLeft(final IbisIdentifier peer) {
			// Ignore, we don't want to delete peers, since we need their stats.
		}
	...
	private boolean registerNewAndDeletedPeers() {
		...
		while (true) {
			...
			pingAdministration.registerPeerLeft(peer);
			changes = true;
		}
		while (true) {
	...
	public void run() {
		...
		pingAdministration.printStatistics(Globals.log.getPrintStream());
